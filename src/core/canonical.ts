const sortCanonical = (value: unknown): unknown => {
  ...
  for (const key of Object.keys(value).sort()) {
    sorted[key] = sortCanonical((value as Readonly<Record<string, unknown>>)[key]);
  }
  return sorted;
};
...
/**
 * Synchronous SHA-256 used by both the Node runtime and browser-side replay.
 * Keeping this implementation platform-neutral lets domain reducers and
 * receipt renderers share one deterministic hash contract.
 */
...
export const hashCanonical = (value: unknown): string => sha256(canonicalize(value));
